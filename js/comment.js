...
export const comment = (() => {
  ...
  const remove = async (button) => {
    ...
    const like = document.querySelector(
      `[onclick="like.like(this)"][data-uuid="${id}"]`
    );
    like.disabled = true;
    ...
    document
      .querySelectorAll('a[onclick="comment.showOrHide(this)"]')
      .forEach((n) => {
  ...
  const update = async (button) => {
    ...
    if (
      id &&
      form.value === form.getAttribute("data-original") &&
    ...
    if (status) {
      ...
      document.getElementById(`content-${id}`).innerHTML =
        card.convertMarkdownToHTML(util.escapeHtml(form.value));
  ...
  const send = async (button) => {
    ...
    if (!name || !commentText || presence === "0") {
      alert("Silahkan isi semua form yang tersedia");
      return;
    }

    // if (presence === "0") {
    //   alert("Silakan pilih status yang valid!"); // Show alert if value is "0"
    //   return;
    // }
    ...
    if (presence === "1") {
      ...
      alert("Silakan pilih status terlebih dahulu!"); // Show alert if no selection
    }
    ...
    fetch(
      "https://script.google.com/macros/s/AKfycbzRPOqbbH_4DfABNMdw7nXDrTTEbHcslivEq-uwSDUmUE48LvbodEFkpiQIjzC9yDk/exec",
      {
        method: "POST",
        headers: {
      ...
      .then((responseData) => {
        ...
        document.getElementById("form-presence").disabled = true;
  ...
  const reply = (button) => {
    ...
    document
      ...
      .insertAdjacentElement("afterend", card.renderReply(id));
  };
  ...
  const edit = async (button) => {
    ...
    await request(HTTP_GET, "/api/comment/" + id)
      ...
      .then((res) => {
        ...
        //     "afterend",
        //     card.renderEdit(id, res.data.presence)
        //   );
        // document.getElementById(`form-inner-${id}`).value = res.data.comment;
        // document
        //   .getElementById(`form-inner-${id}`)
        //   .setAttribute("data-original", res.data.comment);
      });
    ...
    fetch(`https://freeipapi.com/api/json/${comment.ip}`)
      ...
      .then((res) => {
        ...
        document.getElementById(
          `ip-${comment.uuid}`
        ).innerHTML = `<i class="fa-solid fa-location-dot me-1"></i>${util.escapeHtml(
          comment.ip
      ...
      .catch((err) => {
        document.getElementById(
          ...
          comment.ip
        )} <strong>${util.escapeHtml(err.message)}</strong>`;
      });
  };
